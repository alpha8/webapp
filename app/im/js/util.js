    ...
    switch (msg.type) {
        ...
        case 'image':
            if(msg.status === -1){
                ...
                str = '<img class="view-img" layer-pid="" layer-src="' + msg.file.url + '" src="' + msg.file.url + '?imageView&thumbnail=200x0&quality=85" onclick="viewImage(this)" />';
            }
            break;
        case 'file':
            if(msg.status === -1){
...
function viewImage(o){
  var self = $(o);
  var imgs = [];
  imgs.push({"src": self.attr("layer-src"), "thumb": self.attr("src")});

  $("#chatContent .view-img").each(function(i, item){
    if($(item).attr("src") != self.attr("src")){
        imgs.push({"src": $(item).attr("layer-src"), "thumb": $(item).attr("src")});
    }
  });
