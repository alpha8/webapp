var ctx = Yihu.constants.webCtx, psCtx = Yihu.constants.psCtx, cmsCtx = Yihu.constants.cmsCtx, pivot = Yihu.constants.searchPivot.tea;
...
(function($) {
  $(document).ready(function(){
    ...
    Yihu.doGet(cmsCtx + '/datadic/childrens?parentPath={0}'.format(pivot), {}, function(data){
      ...
      Yihu.doGet(ctx + '/artwork/search/tea/records', {}, function(data){
        if(data){
...
var Search = window.Search = {
  ...
  load: function(){
    wf = new Waterfall('#goods>.waterfall', {
      ...
      params: {pageSize: 10},
      path: function(page) {
        return ctx + '/artwork/list?artworkTypeName=tea&currentPage='+page;
      }
    });
  },
  ...
  search: function(){
    ...
    wf.option({
      params: { pageSize: 10, artworkTypeName: 'tea', categoryParentName: type, categoryName: subType, sceneName: scene, subjectName: theme, price: price, shelfTime: duration, keyword: keyword},
