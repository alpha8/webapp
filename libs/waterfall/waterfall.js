...
;(function( $, window, document, undefined ) {
    ...
    var $window = $(window),
        ...
        defaults = {
            ...
            bufferPixel: -400, // decrease this number if you want scroll to fire quicker
            ...
            checkImagesLoaded: false, // triggers layout when images loaded. Suggest false
    ...
    Waterfall.prototype = {
        ...
        layout: function($content, callback) {
            ...
            this.$element.height(Math.max.apply({}, this.colHeightArray) + 300);
        ...
        _requestData: function(callback) {
            ...
            if ( maxPage !== undefined && curPage > maxPage ){
              if(options.hasMore){
                ...
                var btnMore = self._buildMoreBtn(false, options);
                btnMore.bind('click touchstart', function(e){
                  ...
                  $(this).remove();
                  // self.colHeightArray[options.colIndex] -= 200;
         ...
         * 构建加载更多和结束按钮
         */
        _buildMoreBtn: function(isEnd, options){
          var box;
          if(isEnd){
            box = '<div class="more"><button class="btn">没有了!</button></div>';
          }else{
            box = '<div class="more">' + 
                    '<button class="btn" id="btn-more-{0}">展开更多(还有<span class="num">{1}</span>幅) <i class="fa fa-caret-down"></i></button></div>';
            var loadedSize = (options.state.curPage-1) * options.pageSize;
            box = box.format(this.options.prefix, options.totalRecords - loadedSize);
          }
          var top = Math.max.apply({}, this.colHeightArray);
          return $(box).css({
            "position": "absolute",
            "top": top + 30,
            "left": 0,
        ...
        _handleResponse: function(data, callback) {
            ...
            if(data.totalPages && data.totalPages > options.maxPage){
              options.hasMore = true;
            }else{
              options.hasMore = false;
            }
            options.totalRecords = data.totalRecords;
            ...
            // console.log("totalPage: {0}, curPage: {1}, maxPage: {2}".format(data.totalPages, options.state.curPage, options.maxPage));
            if(data.totalPages && options.state.curPage >= data.totalPages){
              options.state.hasNext = false;
              self.append(self._buildMoreBtn(true, options));
